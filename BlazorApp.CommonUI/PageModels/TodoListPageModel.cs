...
namespace BlazorApp.CommonUI.PageModels
{
    public class TodoListPageModel : ComponentBase
    {
        ...
        protected List<TodoModel> TodoModelList { get; set; } = new List<TodoModel>();

        protected TodoModel Todo { get; set; } = new TodoModel();
        ...
        private async Task ReadAsync()
        {
            ...
            if (apiResponse.StatusCode == StatusCodes.Status200OK)
            {
                ...
                TodoModelList = JsonConvert.DeserializeObject<TodoModel[]>(apiResponse.Result.ToString()).ToList();
            }
            else
            {
        ...
        protected async void UpdateAsync(TodoModel todo)
        {
            try
            {
                ...
                var apiResponse = await Http.PutJsonAsync<ApiResponse>("api/todo", todo);
        ...
        protected async Task CreateAsync()
        {
            ...
            try
            {
                var apiResponse = await Http.PostJsonAsync<ApiResponse>("api/todo", Todo);
                if (apiResponse.StatusCode == StatusCodes.Status200OK)
                {
                    MatToaster.Add(apiResponse.Message, MatToastType.Success);
                    Todo = JsonConvert.DeserializeObject<TodoModel>(apiResponse.Result.ToString());
